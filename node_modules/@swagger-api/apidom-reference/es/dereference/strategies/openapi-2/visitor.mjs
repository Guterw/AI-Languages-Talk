...
import Reference from "../../../Reference.mjs"; // @ts-ignore
...
const OpenApi2DereferenceVisitor = stampit({
  ...
  methods: {
    ...
    async ReferenceElement(referencingElement, key, parent, path, ancestors) {
       ...
       */
      if ((isExternalReference || isReferenceElement(referencedElement) || ['error', 'replace'].includes(this.options.dereference.circular)) && !ancestorsLineage.includesCycle(referencedElement)) {
    ...
    async PathItemElement(referencingElement, key, parent, path, ancestors) {
       ...
       *  3. We are dereferencing the fragment lazily/eagerly depending on circular mode
       */
      if ((isExternalReference || isPathItemElement(referencedElement) && isStringElement(referencedElement.$ref) || ['error', 'replace'].includes(this.options.dereference.circular)) && !ancestorsLineage.includesCycle(referencedElement)) {
        // append referencing reference to ancestors lineage
        directAncestors.add(referencingElement);
        const visitor = OpenApi2DereferenceVisitor({
          reference,
    ...
    async JSONReferenceElement(referencingElement, key, parent, path, ancestors) {
       ...
       *  3. We are dereferencing the fragment lazily/eagerly depending on circular mode
       */
      if ((isExternalReference || isJSONReferenceElement(referencedElement) || ['error', 'replace'].includes(this.options.dereference.circular)) && !ancestorsLineage.includesCycle(referencedElement)) {
        // append referencing reference to ancestors lineage
        directAncestors.add(referencingElement);
        const visitor = OpenApi2DereferenceVisitor({
          reference,
