...
const OpenApi2DereferenceVisitor = (0, _stampit.default)({
  props: {
    indirections: [],
    namespace: null,
    reference: null,
  ...
  methods: {
    ...
    async JSONReferenceElement(referencingElement, key, parent, path, ancestors) {
      // skip current referencing element as it's already been access
      if (this.indirections.includes(referencingElement)) {
        return false;
      }
       ...
       *  3. We are dereferencing the fragment lazily/eagerly depending on circular mode
       */
      if ((isExternalReference || (0, _apidomNsOpenapi.isJSONReferenceElement)(referencedElement) || ['error', 'replace'].includes(this.options.dereference.circular)) && !ancestorsLineage.includesCycle(referencedElement)) {
        // append referencing reference to ancestors lineage
        directAncestors.add(referencingElement);
        const visitor = OpenApi2DereferenceVisitor({
          reference,
