...
const AsyncApi2DereferenceVisitor = (0, _stampit.default)({
  ...
  methods: {
    ...
    async ReferenceElement(referencingElement, key, parent, path, ancestors) {
       ...
       */
      if ((isExternalReference || (0, _apidomNsAsyncapi.isReferenceElement)(referencedElement) || ['error', 'replace'].includes(this.options.dereference.circular)) && !ancestorsLineage.includesCycle(referencedElement)) {
    ...
    async ChannelItemElement(referencingElement, key, parent, path, ancestors) {
       ...
       * Cases to consider:
       *  1. We're crossing document boundary
       *  2. Fragment is a Channel Item Object with $ref field. We need to follow it to get the eventual value
       *  3. We are dereferencing the fragment lazily/eagerly depending on circular mode
       */
      if ((isExternalReference || (0, _apidomNsAsyncapi.isChannelItemElement)(referencedElement) && (0, _apidomCore.isStringElement)(referencedElement.$ref) || ['error', 'replace'].includes(this.options.dereference.circular)) && !ancestorsLineage.includesCycle(referencedElement)) {
        // append referencing reference to ancestors lineage
        directAncestors.add(referencingElement);
        const visitor = AsyncApi2DereferenceVisitor({
          reference,
